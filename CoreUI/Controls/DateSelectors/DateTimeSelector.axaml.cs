...
[TemplatePart(PART_TimeSelector, typeof(TimeSelector))]
public class DateTimeSelector : DateSelectorBase
{
    public const string PART_Button = "PART_Button";
    public const string PART_Popup = "PART_Popup";
    public const string PART_TextBox = "PART_TextBox";
    public const string PART_Calendar = "PART_Calendar";
    public const string PART_TimeSelector = "PART_TimeSelector";

    public static readonly StyledProperty<DateTime?> SelectedDateProperty =
        AvaloniaProperty.Register<DateTimeSelector, DateTime?>(
            nameof(SelectedDate), defaultBindingMode: BindingMode.TwoWay);

    public static readonly StyledProperty<string?> WatermarkProperty =
        AvaloniaProperty.Register<DateTimeSelector, string?>(
            nameof(Watermark));
    ...
    private TimeSelectorPresenter? _timeSelectorPresenter;

    static DateTimeSelector()
    {
        DisplayFormatProperty.OverrideDefaultValue<DateTimeSelector>(CultureInfo.InvariantCulture.DateTimeFormat.FullDateTimePattern);
        SelectedDateProperty.Changed.AddClassHandler<DateTimeSelector, DateTime?>((Selector, args) =>
    ...
    private void SyncSelectedDateToText(DateTime? date)
    {
        if (date is null)
        {
            ...
            _timeSelectorPresenter?.SetValue(TimeSelectorPresenter.TimeProperty, null);
        }
        else
        {
            ...
            _timeSelectorPresenter?.SetValue(TimeSelectorPresenter.TimeProperty, date.Value.TimeOfDay);
        }
    }
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        TimeSelectorPresenter.SelectedTimeChangedEvent.RemoveHandler(OnTimeSelectedChanged, _timeSelectorPresenter);
        _button = e.NameScope.Find<Button>(PART_Button);
        e.NameScope.Find<Popup>(PART_Popup);
        _textBox = e.NameScope.Find<TextBox>(PART_TextBox);
        _calendar = e.NameScope.Find<CalendarView>(PART_Calendar);
        _timeSelectorPresenter = e.NameScope.Find<TimeSelectorPresenter>(PART_TimeSelector);
        ...
        TimeSelectorPresenter.SelectedTimeChangedEvent.AddHandler(OnTimeSelectedChanged, _timeSelectorPresenter);
    ...
    private void OnTextBoxPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        if (_calendar is not null)
        {
            ...
            _timeSelectorPresenter?.SetValue(TimeSelectorPresenter.TimeProperty, SelectedDate?.TimeOfDay);
        }
    ...
    private void SetSelectedDate(bool fromText = false)
    {
        ...
        else if (DisplayFormat is null || DisplayFormat.Length == 0)
        {
            if (DateTime.TryParse(_textBox?.Text, out var defaultTime))
            {
                ...
                _timeSelectorPresenter?.SetValue(TimeSelectorPresenter.TimeProperty, defaultTime.TimeOfDay);
            }
        }
        else
        {
                    ...
                    out var date))
            {
                ...
                _timeSelectorPresenter?.SetValue(TimeSelectorPresenter.TimeProperty, date.TimeOfDay);
            }
            else
            {
                SetCurrentValue(SelectedDateProperty, null);
