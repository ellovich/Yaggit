...
[TemplatePart(PART_Presenter, typeof(TimeSelectorPresenter))]
[TemplatePart(PART_Button, typeof(Button))]
public class TimeSelector : TimeSelectorBase, IClearControl
{
    ...
    public static readonly StyledProperty<TimeSpan?> SelectedTimeProperty =
        AvaloniaProperty.Register<TimeSelector, TimeSpan?>(
            nameof(SelectedTime), defaultBindingMode: BindingMode.TwoWay);

    public static readonly StyledProperty<string?> WatermarkProperty = AvaloniaProperty.Register<TimeSelector, string?>(
        nameof(Watermark));

    private bool _suppressTextPresenterEvent;

    private Button? _button;
    private TimeSelectorPresenter? _presenter;
    private TextBox? _textBox;

    static TimeSelector()
    {
        SelectedTimeProperty.Changed.AddClassHandler<TimeSelector, TimeSpan?>((Selector, args) =>
            Selector.OnSelectionChanged(args));
        DisplayFormatProperty.Changed.AddClassHandler<TimeSelector, string?>((Selector, args) => Selector.OnDisplayFormatChanged(args));
    }
    ...
    public void Clear()
    {
        ...
        _presenter?.SetValue(TimeSelectorPresenter.TimeProperty, null);
    }
    ...
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e)
    {
        ...
        TimeSelectorPresenter.SelectedTimeChangedEvent.RemoveHandler(OnPresenterTimeChanged, _presenter);

        _textBox = e.NameScope.Find<TextBox>(PART_TextBox);
        e.NameScope.Find<Popup>(PartNames.PART_Popup);
        _presenter = e.NameScope.Find<TimeSelectorPresenter>(PART_Presenter);
        ...
        TimeSelectorPresenter.SelectedTimeChangedEvent.AddHandler(OnPresenterTimeChanged, _presenter);

        // SetCurrentValue(SelectedTimeProperty, DateTime.Now.TimeOfDay);
        _presenter?.SetValue(TimeSelectorPresenter.TimeProperty, SelectedTime);
        SyncTimeToText(SelectedTime);
    }
    ...
    private void OnSelectionChanged(AvaloniaPropertyChangedEventArgs<TimeSpan?> args)
    {
        ...
        _presenter?.SetValue(TimeSelectorPresenter.TimeProperty, args.NewValue.Value);
